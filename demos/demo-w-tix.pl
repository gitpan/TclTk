...
$interp->call("tixForm",".l",-left=>'%0',-right=>'%100',-top=>'%0');
my $t = text(".t", -font => "-*-Arial Unicode MS--R---*-350-*-*-*-*-*-*", -width => 25, -height=>8);
$interp->call("tixForm",".t",-left=>'%0',-right=>'%100 -20',-top=>'.l',-bottom=>'%100 -20');
$t->insert('end',"qwerty\x{0431}\x{0432}\x{0432}\x{5678}\x{5ab3}");
$|=1;
button(".b", -text => "test", -command=>sub {print "uahaha! It worked!"});
$interp->call("tixForm",".b",-left=>'%0',-right=>'%50',-top=>'.t',-bottom=>'%100');
my $r="aaaa";
button(".d", -textvariable => \$r, -command=>sub {$r++});
$interp->call("tixForm",".d",-left=>'%50',-right=>'%100',-top=>'.t',-bottom=>'%100');
MainLoop;
